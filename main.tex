...
\textbf{x} = [$K_{LA}$    $x_{LA}$  $K_{long}$]
 \end{center}
and their respective simulation results $f($\textbf{x}$)$ $\in \mathbb{R}^4$
 \begin{center}
$f(\textbf{x})$ = [$e$    $\Delta U_{x}$  $a_y$   $a_x$]
 \end{center}
...
The cost function designed for this problem is designed with the objective of reducing $e$ and $\Delta U_x$ while minimizing $a_x$ and $a_y$ such as to reduce the jerkiness motion experienced by the passengers. However, these requirements are not equally weighted, it is more important to ensure the vehicle follows the trajectory than ensuring comfortable ride. This in turn must be represented in the objective function.
\[p_{qaudratic}(\textbf{x}) = \Sigma \theta_i max(g_i(x),0)^2\]
\[f_{obj}(\textbf{x}) = \sum_{i=1}^{4} f_i(\textbf{x}) + p_{qaudratic}(\textbf{x})\]
