...
and their respective simulation results f($\textbf{x}$) $\in \mathbb{R}^4$:
 \begin{center}
[$e$    $\Delta U_{x}$  $a_y$   $a_x$]
 \end{center}
 to accurately predict the results for any given $\textbf{x}$. Once this regression model has been established it is then possible to optimise the for the ideal design variable $x^*$. The optimization problem is formally stated as:\\
\begin{equation}
\begin{aligned}
